...
int GetElem(PNODE L, int i, ElemType *e)
{
	/* j为计数器 */
	int j = 1;
	/* p指向第一个结点 */
 ...
 * 若这样的数据元素不存在，则返回值为0 */
int LocateElem(PNODE L, ElemType e, int(*compare)(ElemType, ElemType))
{
	int i = 0;
	/* p指向第1个元素 */
	PNODE p = L->next;
...
int PriorElem(PNODE L, ElemType cur_e, ElemType *pre_e)
{
	/* p指向第2个元素 */
	PNODE p = L->next->next;
	/* p没到表头 */
...
int NextElem(PNODE L, ElemType cur_e, ElemType *next_e)
{
	/* p指向第2个元素 */
	PNODE p = L->next->next;
	/* p没到表头 */
...
// i的合法值为1≤i≤表长+1 */
int ListInsert(PNODE L, int i, ElemType e)
{
	PNODE p, s;
	/* i值不合法 */
...
int ListDelete(PNODE L, int i, ElemType *e)
{
	PNODE p;
	/* i值不合法 */
...
void ListTraverse(PNODE L, void(*visit)(ElemType))
{
	/* p指向头结点 */
	PNODE p = L->next;
	while (p != L)
...
void ListTraverseBack(PNODE L, void(*visit)(ElemType))
{
	/* p指向尾结点 */
	PNODE p = L->prior;
	while (p != L)
